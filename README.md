	...
	public Retorno salvar(String usuario, Fiador fiador) {
    	...
    	@Override
    public Retorno salvar(String usuario, Fiador fiador) throws FESException {
        // inclui ou atualiza
        List<Fiador> fiadoresCadastrados = consultarFiadores(usuario, fiador.getCodigoFies());
        if (fiadoresCadastrados.size() > 1) {
            Retorno wR = new Retorno();
            wR.setCodigo(-1L);
            wR.setMensagem("Não é permitido cadastrar mais de 2 fiadores.");
        }
        ...
        if (dadosCpfRetorno.getSituacaoCPF() == 0) {
            Retorno wR = new Retorno();
            wR.setCodigo(-1L);
            wR.setMensagem("CPF informado com restrição. Favor regularizar o CPF para continuar a contratação!");
            return wR;
        } else {
            return salvar("I", usuario, fiador);
        }
